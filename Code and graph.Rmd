...
author: "Michelle Dai, Lucy Tian, Eli Wang"
date: "2022-11-05"
output: pdf_document
---
...
We first examine the total number of missing values per column. The 5% threshold is 260. We see only missing values in `sc1` and `sc2` with count greater than the threshold. Thus, for columns with missing value count less than the threshold, one method would be to use the Complete Case Analysis or use multiple imputing to help dealing with these missing data (we consider multiple imputing to be more achievable here as MICE would help us dealing with this).
...
From the pattern plot above, among the 14 patterns of missing data, only the top 4 has significant count of missing values. Hence, we use Multivariate Imputation with 4 imputations.
...
# we also tried to incorporate quadratic bmi as commented by Nicholas
data_imputed$bmi_sq = (data_imputed$bmi)^2

data_imputed <- data_imputed %>% mutate(age_diag = if_else(cexam == 0, age+2*15, age+2*(cexam-1)))
```

### Logistic Lasso

We apply Logistic Lasso here to narrow down our lists of covariates.

```{r}
library(glmnet)
y = as.matrix(data_imputed$diag)

x = as.matrix(dplyr::select(data_imputed, c(as.factor('sex'), 'age', 'sc1', 'sc2', 'dbp', 'sbp', 'smoke', 'bmi', 'bmi_sq')))
...
# to check assumption for non-normal residuals
hist(y-strtoi(predict(best_model, newx = x, type = 'class')))
```
...
Then we used our covariates of selection to fit logistic regression model.
...
table(data_imputed$diag) # check assumption

x = as.matrix(dplyr::select(data_imputed, c(as.factor('sex'), 'age_diag', 'sc1', 'sc2', 'dbp', 'sbp', 'smoke', 'bmi')))

mod = multinom(diag~as.factor(sex)+age_diag+sc1+sc2+dbp+sbp+smoke+bmi, data = data_imputed, family = "binomial")
summary(mod)
...
hoslem.test(data_imputed$diag, strtoi(predict(mod, newx = x, type = 'class')), g=7)

library(pROC)

predprob <- predict(mod, type= "probs")
roccurve <- roc(data_imputed$diag ~ predprob)
